...
var Kothic = function(devicePixelRatio,mapcss){
  this._devicePixelRatio = (devicePixelRatio) ? devicePixelRatio : 1;
  this.mapcss = (mapcss)?mapcss: (new MapCSS());
...
Kothic.prototype.importStyle = function(filePath,styleName){
  require(path.join(filePath,styleName)).style(this.mapcss);
  if (fs.fileExists(path.join(filePath,styleName,'.png'))){
    this.mapcss.preloadSpriteImage(styleName,path.join(filePath,styleName,'.png'));
  }
}
