...
var TextIcons=function(kothic){
  this.kothic = kothic;
}


TextIcons.prototype.render = function (ctx, feature, collides, ws, hs, renderText, renderIcon) {
  var style = feature.style, img, point, w, h;

  if (renderIcon || (renderText && feature.type !== 'LineString')) {
    var reprPoint = this.kothic.geom.getReprPoint(feature);
    if (!reprPoint) {
      return;
    }
    point = this.kothic.geom.transformPoint(reprPoint, ws, hs);
  }

  if (renderIcon) {

    img = this.kothic.mapcss.getImage(style['icon-image']);
  ...
  if (renderText) {
    this.kothic.style.setStyles(ctx, {
      ...
      font: this.kothic.style.getFontString(style['font-family'], style['font-size'])
    });
    ...
    if (feature.type === 'Polygon' || feature.type === 'Point') {
      ...
      var points = this.kothic.geom.transformPoints(feature.coordinates, ws, hs);
      this.kothic.textOnPath(ctx, points, text, halo, collides);
    }
  }
